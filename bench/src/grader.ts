 ...
 * task. Adapted from flutter-axi's bench grader — domain-agnostic logic with
 * an error-triage rubric.
 */
...
export function buildGradingPrompt(
  ...
  return `You are a benchmark grader evaluating whether an AI agent completed a Sentry error-triage task against a live Sentry project.

TASK: ${taskPrompt}
${hintSection}
CONTEXT: The agent runs inside a scratch workspace directory; relative output
paths like ./file.txt correctly resolve against that workspace (wherever the
trajectory shows it to be), not against any other directory. The Sentry
project is a seeded fixture, so the KNOWN FACTS above are authoritative.

AGENT TRAJECTORY:
${trajectory}

Rules:
- PASS if the agent actually queried the live Sentry project (listed issues, fetched an issue/event/stack trace, ran a search, ran Seer, read performance data as required) AND produced a correct, complete answer
- FAIL if the agent invented issue titles, counts, files, releases, or stack frames without a tool/command output backing them
- FAIL if the agent ran the right commands but misread their output (e.g. reported the most *frequent* issue when asked for the one affecting the most *users*)
...
- For mutating tasks (resolve/ignore/assign), PASS only if the write actually succeeded against the correct issue AND the agent confirmed the new state rather than assuming it
- Approximate numeric answers are acceptable only when the task asks for an approximation; exact counts must match
